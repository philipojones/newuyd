...
class Program(Base):
    ...
    category = Column(String, index=True)  # education, agribusiness, leadership, environment, tourism, lifeskills
    content = Column(Text)
...
class Event(Base):
    ...
    title = Column(String, index=True)
    description = Column(Text)
    event_type = Column(String, index=True)  # workshop, seminar, competition, conference, etc.
...
async def update_program(program_id: int, program: ProgramCreate, db: Session = Depends(get_db)):
...
async def get_news(
    ...
    news = query.order_by(NewsArticle.publish_date.desc()).offset(skip).limit(limit).all()
    return news
...
async def get_latest_news(db: Session = Depends(get_db)):
    ...
    ).order_by(NewsArticle.publish_date.desc()).limit(10).all()
    return news
...
async def get_featured_news(db: Session = Depends(get_db)):
    ...
    ).order_by(NewsArticle.publish_date.desc()).limit(5).all()
    return news
